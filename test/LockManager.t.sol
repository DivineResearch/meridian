...
contract LockManagerTest is BaseTest {
    ...
    function test_initialize_invalidPermit2_reverts() public {
        LockManager implementation = new LockManager();

        vm.expectRevert(ILockManager.Permit2Invalid.selector);
        new ERC1967Proxy(address(implementation), abi.encodeCall(LockManager.initialize, (owner, address(0))));
    }
    ...
    function test_execute_notPartner_reverts() public {
        vm.prank(owner);
        lockManager.setPartnerStatus(partner, true);
        ...
        ISignatureTransfer.SignatureTransferDetails memory transferDetails =
            ISignatureTransfer.SignatureTransferDetails({to: alice, requestedAmount: 100e6});
